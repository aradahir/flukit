...
@app.command(no_args_is_help=True, help = "Phylogenetics analysis, plot and find representative viruses (all genes)")
def tree(
    sequences: Path = typer.Option(
        ...,
        "-s",
    ...
    # call clades
    clades = run.call_clades(sequences, lineage)
    ...
    #call phylogenetics
    run.tree(sequences, lineage)
    #virus_table.to_csv('resul_tmp.csv')
    results.insert(0, 'seqName', results.pop('seqName')) # reorder 
